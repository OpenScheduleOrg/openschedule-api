...
from . import bp_api
from ..models import session, select, delete, result_to_json, Consulta, Cliente, Clinica, Horario, Notification
...
@bp_api.route("/consultas", methods=["POST"])
def post_consulta():
    ...
    try:
        ...
        session.add(
            Notification(
                ...
                message=
                "Você possui uma nova consulta marcada para %s, %s de %s as %s."
                % (WEEKDAY[marcada.weekday()].lower(), marcada.day,
                   MONTH[marcada.month].lower(), marcada.strftime("%H:%M"))))

        session.commit()
        session.refresh(consulta)

        return jsonify(status="success", data={"consulta":
                                               consulta._asjson()}), 201
...
@bp_api.route("/bot/consultas", methods=["POST"])
def bot_post_consulta():
    body = request.get_json()
...
@bp_api.route("/bot/consultas/<int:id>", methods=["GET"])
@bp_api.route("/bot/consultas", methods=["GET"])
def get_consultas(id=None):
    ...
    if id is not None:
        ...
        date_start, date_end = valid_date_params(date_start=date_start,
                                                 date_end=date_end)
        try:
            ...
            stmt = stmt.order_by(Consulta.marcada)
...
@bp_api.route("/consultas/<int:id>", methods=["PUT"])
def update_consulta(id):
    ...
    try:
        ...
        marcada = consulta.marcada
        if marcada != old_marcada:
            session.add(
                Notification(
                    cliente_id=consulta.cliente_id,
                    message=
                    "Sua consulta que estava marcada para %s de %s as %s, foi alterada para %s, %s de %s as %s."
                    %
                    (old_marcada.day, MONTH[old_marcada.month].lower(),
                     old_marcada.strftime("%H:%M"),
                     WEEKDAY[marcada.weekday()].lower(), marcada.day,
                     MONTH[marcada.month].lower(), marcada.strftime("%H:%M"))))
    ...
    return jsonify(status="success", data={"consulta":
                                           consulta._asjson()}), 200


@bp_api.route("/bot/consultas/<int:id>", methods=["PUT"])
def bot_update_consulta(id):
    body = request.get_json()
...
def delete_consulta(id):
    ...
    try:
        consulta = session.execute(
            select(Consulta.marcada,
                   Consulta.cliente_id).where(Consulta.id == id)).first()
        marcada = consulta[0]
        ...
        session.add(
            Notification(
                cliente_id=consulta[1],
                message=
                "Sua consulta marcada para %s, %s de %s as %s foi cancelada." %
                (WEEKDAY[marcada.weekday()].lower(), marcada.day,
                 MONTH[marcada.month].lower(), marcada.strftime("%H:%M"))))
...
@bp_api.route("/bot/consultas/<int:id>", methods=["DELETE"])
def bot_delete_consulta(id):
